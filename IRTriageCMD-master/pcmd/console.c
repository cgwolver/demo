...
VOID ConInString(LPTSTR lpInput, DWORD dwLength)
{
    ...
    cmd_free(pBuf);
#endif
    /*  strip backspace and delete from console input */
	pBuf=lpInput;
    for (p = lpInput; *p; p++)
    {
		if (p!=lpInput) *lpInput=*p;
		if (*p == _T('\x0d')) {
            break;
		} else if (*p == _T('\x08') ) {
			/* do not go before beginning of string */
			if (lpInput!=pBuf) lpInput--;
		} else if (*p != _T('\x7f') ) {
			lpInput++;
        }
    }
...
VOID ConPrintf(LPTSTR szFormat, va_list arg_ptr, DWORD nStdHandle)
{
    ...
    len = (DWORD)_vstprintf(szOut, OUTPUT_BUFFER_SIZE,szFormat, arg_ptr);
...
INT ConPrintfPaging(BOOL NewPage, LPTSTR szFormat, va_list arg_ptr, DWORD nStdHandle)
{
    ...
    len = _vstprintf(szOut,OUTPUT_BUFFER_SIZE,szFormat, arg_ptr);
