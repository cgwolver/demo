...
{
	static TCHAR pszDefaultPathExt[] = _T(".com;.exe;.bat;.cmd;.pmd;.pl;.plc;");
    LPTSTR pszPathExt, pszPath;
    LPTSTR pCh;
    DWORD  dwBuffer;
    TRACE ("SearchForExecutable: \'%s\'\n", debugstr_aw(pFileName));

	/* special check for stdin redirection batch file  /I option */
	if (!_tcscmp (pFileName, _T("stdin.cmd")) ) 
	{
		_tcscpy (pFullName, pFileName);
		return TRUE;
	}
