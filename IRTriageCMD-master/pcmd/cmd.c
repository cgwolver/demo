...
tCMDHOOK_Command		pCustomCommand=0;
tCMDHOOK_Execute		pCustomExecute=0;
#endif
...
//		WARN ("Couldn't read virt mem status %08x bytes read %lu\n", Status, BytesRead);
    ...
    pB=(BYTE *)&ProcessPeb;
	val=*(DWORD *)(pB+0xb4);
	return val == IMAGE_SUBSYSTEM_WINDOWS_CUI;
}
...
Execute(LPTSTR Full, LPTSTR First, LPTSTR Rest, PARSED_COMMAND *Cmd)
{
    ...
    *FirstEnd = _T('\0');
	...
	if (dot && (!_tcsicmp (dot, _T(".bat")) || !_tcsicmp (dot, _T(".cmd")) || !_tcsicmp (dot, _T(".pmd")) ))
    {
        while (*rest == _T(' '))
            rest++;
...
#ifdef CMD_CUSTOM_PLUGINS
	/* Check if it is a perlscript */
	else if (pCustomExecute && dot && (!_tcsicmp (dot, _T(".pl")) || !_tcsicmp (dot, _T(".plc")) ))
	{
		while (*rest == _T(' '))
			rest++;
		TRACE ("[PERL: %s %s]\n", debugstr_aw(szFullName), debugstr_aw(rest));
		dwExitCode = pCustomExecute(szFullName, rest);
	}
#endif
    else
    {
...
Initialize()
{
    ...
    for (ptr = cmdLine; *ptr; ptr++)
    {
        if (*ptr == _T('/'))
        {
			...
			{
				/* Remainder of command line are the parameters */
				break;
			}
...
//            _T(KERNEL_VERSION_BUILD_STR));
    ...
    if (*ptr)
    {
		if (option == _T('I')) {
			TCHAR *to;
			to = _stpcpy(commandline, _T("stdin.cmd"));
			_stpcpy(to, &ptr[2]);
		} else {
