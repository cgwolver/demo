...
#if 1
static TCHAR *g_cmdline = NULL;

TCHAR *
str_append(TCHAR *a, TCHAR *b)
{
	BOOL first_init = 0;
	size_t existing_len = a != NULL ? _tcslen(a) : 0;
	TCHAR* ta = NULL;
	if (existing_len)
	{
		ta = malloc(1 + existing_len + _tcslen(b));
		_tcscpy(ta, a);
	}
	a = realloc(a, 1 + existing_len + _tcslen(b));
...
LPTSTR WINAPI Cmdx_GetCommandLine(void) {
	return g_cmdline;
}

VOID WINAPI Cmdx_SetCommandLine(int argc, TCHAR **argv) {
	int N;
	for (N = 1; N < argc; N++) {
		if (N) {
			g_cmdline = str_append(g_cmdline, _T(" "));
		}
