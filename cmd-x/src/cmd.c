...
void InitJmpBuff(jmp_buf* buf)
{
	memset(buf, 0, sizeof(jmp_buf));
}

int JmpBufIsVaild(jmp_buf* buf )
{
	int i = 0;
	int n = 0;
	n = sizeof(jmp_buf)/sizeof(_JBTYPE);
	
	for (i = 0; i < n; i++)
	{
		if (buf[i]->Part[1] != 0 || buf[i]->Part[2]!=0)
			return 1;
	}
...
void LongJump(jmp_buf*pbuf, int value)
{
	jmp_buf* buf = (jmp_buf*)pbuf;
	int ret = 0;
	ret = JmpBufIsVaild(buf);
	if (ret)
	{
		longjmp(buf, value);
	}
...

{
    ...
    */
	...
	Cmdx_SetCommandLine(argc, szArglist);

	LocalFree(szArglist);

#else
	Cmdx_SetCommandLine(argc, argv);
#endif
...
#ifdef _CMD_X
	InitJmpBuff(MainEnv);
	InitJmpBuff(CmdJBuf1);
	InitJmpBuff(CmdJBuf2);
#endif
